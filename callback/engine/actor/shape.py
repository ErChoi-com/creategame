"""design/part-05-the-work.md §5.6 — Shape: the single canonical Spotlight/CraftContribution resolution.

Three scenes (setup / turn / resolution), each an independent position decision (positions.py).
This is the one place YourSpotlight/CraftContributionScore are computed — reception.py consumes this
...
"peak" term keyed to spikiness, and a −4.0×spikiness CraftContribution penalty) without fully specifying
...
SPOTLIGHT_PEAK_WEIGHT = 0.55
SPOTLIGHT_PERFORMANCE_WEIGHT = 0.45
CRAFT_CONTRIBUTION_SPIKINESS_COEF = 4.0
...
def resolve_shape(
    ...
    spotlight = SPOTLIGHT_PEAK_WEIGHT * peak + SPOTLIGHT_PERFORMANCE_WEIGHT * your_performance_shape + mean_for_you
    craft_contribution = your_performance_shape - CRAFT_CONTRIBUTION_SPIKINESS_COEF * spike + mean_for_film

    total_overspend = sum(s.overspend for s in scene_reads)

    return ShapeResult(spotlight=spotlight, craft_contribution=craft_contribution, spikiness=spike, total_overspend=total_overspend)
