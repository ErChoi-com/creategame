...
| **ux/** | A companion document to `design/` — not systems, but screens. The exact gameplay flow, user experience, and UI for the actor spine: onboarding, the season loop screen by screen, the pull systems (Rolodex/Leverage/Trades), and the visual/feedback language tying them together. Start at `ux/00-index.md`. Nothing in it changes a `design/` formula — it specifies what the player sees and taps, using the numbers and rules `design/` already defines. |
