...
**`franchise.spinoff.actor_launch` is intentionally NOT cataloged as a required coverage ID — see the Flagged Balance Findings section after the Awards section below.** Gated on `SPINOFF_INDISPENSABILITY_THRESHOLD = 55.0` (`simulation/_franchises.py`) AND `you_are_current_lead` — reachable in principle via `launch_spinoff(franchise_id)` (`session.py:799-822`, design/part-09-genres-franchises-and-tie-ins.md §9.5). Re-confirmed still unreachable (0/480 runs) after the Phase 4 round-2 decay change.
...
**Post-round-2 re-check (bug-hunting/tuning follow-up pass):** re-verified directly (not via the coverage tool's gap list, which excludes these five by construction regardless of reachability — see the caveat below) across 480 runs (8 archetypes × 60 seeds × 60 years): all five remain at **0 hits**. The round-2 `PRESTIGE_DECAY`/`AFFECTION_DECAY` softening measurably raised mean ending Prestige (see `04-CHANGELOG.md`) but not enough to cross any of these thresholds within a realistic career. Confirmed still an open finding, not force-fixed further on this pass.

**Caveat for future coverage checks:** because these five IDs are deliberately written as prose (not `| \`id\` |` table rows) so the parser in `load_expected_ids()` never treats them as required, a shrinking gap-count from `run_full_coverage_check`/`check_coverage` does **not** by itself mean they've become reachable — it only reflects the *other*, still-cataloged IDs closing. Verifying one of these five needs a direct `Counter` check against the specific archetype(s) that call it (as done above), not just "the gap list is empty."
...
**`director.franchise.spinoff_pitch` is intentionally NOT cataloged as a required coverage ID — see the Flagged Balance Findings section after the Awards section below.** Same `SPINOFF_INDISPENSABILITY_THRESHOLD = 55.0` gate as the actor-track row above (no `you_are_current_lead` requirement here), reachable via `launch_directing_spinoff(franchise_id)` from `directing_spinoff_options()` (`session.py:698-720`, design/part-09-genres-franchises-and-tie-ins.md §9.5). Re-checked after the Phase 4 round-2 Standing decay change: still 0/39 hits for `director_track` — unlike the actor-track findings below, this one is not just rare, it's structurally blocked by `director_track`'s own franchise-lifecycle logic (reason unchanged from the original finding: staying active enough for indispensability to climb toward 55 and going dormant enough to open the reboot path are mutually exclusive within one franchise). Still flagged, not force-fixed.
