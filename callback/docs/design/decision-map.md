...
| `release.actor.resolved.<strategy>` | Resolved (actual) release outcome — informative secondary tally, not the coverage gate (see "How to read this catalog" above) | Whatever `result["release_label"]` reports, which can differ from the request via `studio_overruled` | `actor/studios.py` (`decide_release_strategy`) via `Session.choose_release()`'s return dict (`session.py:1184-1185,1237-1253`) | design/part-03-design-overview.md §3.4 |
