...
| **9. World layer** | Guilds, health-plan cliff, strikes, festivals, territories, the background industry and trades digest (§10.0) | A player turns down a good role to hit their health-plan threshold. |
...
*(v9 — a second dependency worth calling out the same way: §10.0's trades digest only needs the Rolodex (phase 4) and the calendar (phase 2), not the rest of phase 9's guilds/festivals/territories machinery. It's cheap — a once-a-year pull digest, no push budget cost — and there's no reason to hold it until phase 9 just because that's where the section lives; shipping it as soon as its two real dependencies exist is a legitimate sequencing option.)*
